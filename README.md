...
| `-b, --background <#rrggbb>` | 完全不含透明像素的格子之外，用于合成部分透明像素的颜色 | `#000000` |
| `--opaque` | 关闭透明处理，整个矩形区域都填满 `--background` | 关闭 |
...
含**完全透明**像素的格子不会被绘制，直接露出终端自己的背景。圆形或异形的 Logo 因此不会套着一块实心矩形底，边缘也不会出现一圈描边：
...
- 格子**全是**透明像素 —— 完全不绘制。
- 格子**部分**透明（比如圆形 Logo 的边缘格子，一半是图形、一半是空）—— 图形照常绘制，但背景交给终端。程序无从知道你终端的背景是什么颜色，凭空猜一个反而是错的。

图像自身的内容不受影响 —— 图片里本来就是黑色的部分，仍然渲染为黑色。想让整个矩形区域都填满 `--background`、恢复「不做透明处理」的行为，加 `--opaque`。
